...
### 说明：
- **`src/`**：源代码目录，包含模型、数据处理、训练、评估、API 等功能。
- **`data/`**：存放数据集的目录。`processed/` 用于保存经过处理后的数据，`raw/` 存放原始的训练集、验证集和测试集数据。
- **`logs/`**：存放训练过程中的日志文件，帮助追踪训练进度和效果。
- **`pretrained/`**：存放预训练模型的文件夹，可以存放BERT等预训练模型文件。
- **`requirements.txt`**：项目依赖的库和包列表，确保环境一致性。
- **`config.py`**：项目的配置文件，包含了路径、参数等设置。

### 用法：
你可以通过该项目的结构快速定位到需要的代码模块，帮助你在训练、测试、评估和推理过程中管理各个部分。
